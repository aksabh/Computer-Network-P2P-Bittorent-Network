...
public class Client1 {
	...
	private static Socket connection,connection2;
    private static String fileName;
    

    private static PrintStream os;
    private static int chunks;
    private static final int c1Port = 7001;   //The client will be listening on this port number
    public static ServerSocket c1listener;
    ...
    public static void main(String[] args) throws IOException {
        ...
        //os.println(fileName);
        for(int i=1;i<= chunks;i=i+5,receivedChunks++)
        {
    ...
    public static void peer2peer(int a[], int c) throws IOException{
    	...
    	while(true)
    	{
    		System.out.println("Waiting for connection with neighbour client2");  //wait for connection
    		connectToNeighbour(a);
    		
    		System.out.println("The recevied chunks are:"+receivedChunks);
	...
	public static void connectToNeighbour(int ar[]) throws IOException{
		...
		c1listener = new ServerSocket(c1Port);                                            //create a serversocket
		try
		{
		
				
			 connection2 = c1listener.accept();
             // System.out.println("Conection Accept : " + clientSocket);
              ...
              Thread t = new Thread(new Client1Handler(connection2,chunks,2,ar));
             // clientNum++;
              t.start();
              //connection2.close();
              c1listener.close();
	 ...
	 public static void receiveFileFromNeighbour(String fileName) {
	        ...
	        try {
	            ...
	            System.out.println("File " + fileName + " received from Client5");
	        } catch (IOException ex) {
	          //  Logger.getLogger(ClientConnection.class.getName()).log(Level.SEVERE,
	                 //   null, ex);
	        	//sysout
	  ...
	  public static int downloadFromNeighbour(int an[]){
	    	...
	    	try{
	    		...
	    		connection2 = new Socket("localhost", 7005);	
	            System.out.println("Connected to Client5");
	 ...
	 public static void mergeFiles(int chunkcount,String filename) throws IOException {
		 ...
		 FileOutputStream f = new FileOutputStream("src/client1/"+filename);
         int i=1;
         int length;
     byte[] buffer = new byte[1024];
         while (i<=chunkcount) {

             FileInputStream fs = new FileInputStream("src/client1/"+i);
